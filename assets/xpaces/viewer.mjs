...
export async function mountXpace(host, item, {signal}={}) {
  ...
  let viewer,root,frame=0,observer,disposed=false,pan=false;
  const listeners=[];
  ...
  function dispose(){if(disposed)return;disposed=true;cancelAnimationFrame(frame);observer?.disconnect();for(const off of listeners)off();viewer?.dispose();if(root)release(root);signal?.removeEventListener('abort',dispose);}
  ...
  try {
    ...
    if(!gltf.animations.length)batchStatic(root);
    const snapshot={cols:size.x,rows:size.z,wallHeight:size.y,layout:[],actors:[],moving:true};
    viewer=createLifeRenderer({canvas,snapshot,stockCamera:true,sceneFactory:(input,options)=>{
    ...
    host.xpaceState=()=>({camera:viewer.cameraState,disposed});
    return dispose;
  } catch(error) {
