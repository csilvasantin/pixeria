...
const fields=['id','nombre','tipo','categoria','cantidad','medidas','pantalla','fabricante','modelo','garantia','origen'];
export function exportRows(entries){return entries.filter(e=>e.visible).map(e=>Object.fromEntries(fields.map(k=>[k,e[k]??''])));}
export function toCSV(rows){
 const columns=['id','nombre','tipo','categoria','cantidad','ancho','fondo','alto','unidad','pantalla','fabricante','modelo','garantia','origen'];
...
export function mountInventory(host,item,{manifest,entries},viewer,on,signal){
 ...
 return {select:id=>focus(entries.find(e=>e.id===id)||null,true),dispose:clearOutline,state:()=>({selected:selected?.id||null,items:entries.map(e=>({id:e.id,visible:e.visible,objectVisible:e.object.visible,core:e.origen==='json-medidas'}))}),export:documentData};
}
