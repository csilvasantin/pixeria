...
export function mountInventory(host,item,{manifest,entries},viewer,on,signal){
 ...
 function view(e,{scroll=true}={}){if(!e)return;if(!e.visible){e.visible=true;apply();}focus(e);viewed=e.id;writeURL();if(scroll)host.querySelector('.xpace-stage')?.scrollIntoView({block:'nearest',behavior:'smooth'});viewer.frameObject?.(e.object);pulse(e);}
