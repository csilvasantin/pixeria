...
export function mountInventory(host,item,{manifest,entries,capsulas},viewer,on,signal){
 ...
 function openFicha(e){if(!e){ficha.hidden=true;ficha.innerHTML='';return;}ficha.hidden=false;ficha.innerHTML=fichaMarkup(e);const close=ficha.querySelector('[data-ci-close]');if(close)on(close,'click',()=>openFicha(null));}
 function apply(){for(const e of entries){if(e.object)e.object.visible=e.visible;e.checkbox.checked=e.visible;}for(const [checkbox,rows] of groups){const count=rows.filter(e=>e.visible).length;checkbox.checked=count===rows.length;checkbox.indeterminate=count>0&&count<rows.length;}panel.querySelector('.xpace-count').textContent=`${entries.filter(e=>e.visible).length}/${entries.length} ${en?'visible':'visibles'}`;if(selected&&!selected.visible)focus(null);viewer.invalidateShadows();writeURL();}
