...
**Counting (single source of truth).** An ITIL item (CI) is every IT technology/infrastructure
item in the inventory: categories `IoT` and `Pantallas`, plus `Equipamiento` of type `pos` (POS)
and `ups` (UPS). Furniture, lighting, plants, architecture and kitchen/HVAC machinery do not count.
`itilCatalog()`/`itilCount()` compute it from the inventory, never from what a level draws, and
all 4 levels draw exactly that set: items without their own drawing use the generic «pendiente»
icon (`itil/*/generico.*`, generated by `scripts/itil-generico.py`). The Advanced panel shows the
total and the DEMO breakdown by status (Cafebrería: 19). `test/xpace-itil.test.mjs` checks it.
